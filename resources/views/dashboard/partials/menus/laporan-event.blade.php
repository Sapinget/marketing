            ...
            <div v-if="activeTab === 'laporan_event' && tabDataLoaded['lpjk']" class="space-y-6 animate-fadeIn pb-10">
                ...
                <section class="section-card section-card-body">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-5">
                        <div class="modal-header-copy">
                            <div
                                class="w-12 h-12 rounded-2xl bg-violet-500 text-white flex items-center justify-center">
                                <i class="fa-solid fa-calendar-check text-[16px]"></i>
                            </div>
                            <div>
                                <h2 class="type-title font-bold text-slate-900">Laporan Event</h2>
                                <p class="text-[10px] text-slate-400 uppercase tracking-widest mt-0.5">
                                    Monitoring Budget &amp; Realisasi Event</p>
                            </div>
                        </div>
                        <div class="mobile-toolbar-stack">
                            <div class="relative sm:w-44">
                                ...
                                <input v-model="lpjkSearch" type="text" placeholder="Cari event..."
                    ...
                    <div v-if="lpjkSummary.chips.length" class="chips-grid mt-4 pt-4 border-t border-slate-100">
                        <span v-for="ch in lpjkSummary.chips" :key="ch.label" :class="['inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold', ch.cls]">{{ ch.label }} <span class="opacity-50">&middot;</span> {{ ch.n }}</span>
                    </div>
                </section>
                <div class="md:hidden space-y-3">
                    <div class="space-y-3">
                        ...
                        <div v-for="(row, idx) in pagedLpjkData" :key="'lpjk-mobile-' + (row.ID || idx)"
                            ...
                            <div>
                                ...
                                <p v-if="row.Keterangan" class="type-meta text-slate-400 mt-2 line-clamp-1">{{ row.Keterangan }}</p>
                            </div>
                            <div class="mobile-data-card__summary">
                                <div>
                                    <div class="type-meta text-slate-400 uppercase">Budget</div>
                                    <div class="type-body font-bold text-slate-700">{{ formatCurrency(row.Budget_Rencana) }}</div>
                                </div>
                                <div>
                                    <div class="type-meta text-slate-400 uppercase">Realisasi</div>
                                ...
                                <div class="flex items-center gap-2">
                                    <button @click="openLpjkDetail(row)"
                                        ...
                                        title="Detail Pengeluaran" aria-label="Detail Pengeluaran"><i
                                            class="fa-solid fa-eye text-[10px]"></i></button>
                    ...
                    <div class="flex items-center justify-center gap-2 py-2">
                        ...
                        <span class="px-3 text-[10px] font-bold text-ppp-accent">{{ lpjkPage }} / {{ lpjkTotalPages }}</span>
                ...
                <div class="hidden md:block section-card section-card-shell">
                        ...
                        <table class="w-full text-[10.5px] text-left border-collapse min-w-[900px]">
                            ...
                            <tbody>
                                <tr v-if="pagedLpjkData.length === 0">
                                    <td colspan="8" class="px-4 py-12 text-center text-[11px] text-slate-400">
                                        Belum ada data event</td>
                                </tr>
                                <tr v-for="(row, idx) in pagedLpjkData" :key="row.ID"
                                    class="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                    <td class="px-4 py-3 text-center text-[11px] text-slate-400">{{ (lpjkPage - 1) * 20 + idx + 1 }}</td>
                                    <td class="px-4 py-3">
                                        <p class="font-semibold text-slate-800 uppercase text-[10.5px]">{{ row.Nama_Event }}</p>
                                        <p v-if="row.Keterangan" class="text-[10px] text-slate-400 mt-0.5 line-clamp-1">
                                            {{ row.Keterangan }}</p>
                                    </td>
                                    <td class="px-4 py-3 text-center text-[11px] text-slate-500">{{ formatShortDate(row.Tanggal) }}</td>
                                    <td class="px-4 py-3 text-center text-[11px] font-bold text-slate-600">{{ formatCurrency(row.Budget_Rencana) }}</td>
                                    <td class="px-4 py-3 text-center text-[11px] font-bold text-slate-800">{{ formatCurrency(row.Realisasi_Biaya) }}</td>
                    ...
                    <div class="table-pager-bar-compact">
                        <span class="text-[10px] font-bold text-slate-400">{{ filteredLpjkData.length }}
                            event</span>
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-5">
                        ...
                        <div class="mobile-toolbar-stack">
                            ...
                            <div class="relative flex-1 sm:w-44">
                                <i
                                    class="fa-solid fa-magnifying-glass absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[10px]"></i>
                                <input v-model="adsSearch" type="text" placeholder="Cari iklan..."
                                    class="form-input-search" />
                            </div>
